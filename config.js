...
module.exports = kconfig = async (kill, message) => {
	...
	const isQuotedVideo = quotedMsg && quotedMsg.type === 'video'
		...
		if (chats.match(/kaiser/gi)) {
			kill.reply(from, 'Kaiser ? El mismísimo traga tulas', id)
			} 

	// x2

		if (chats.match(/x2/gi)) {
			kill.reply(from, 'x3', id)
			} 
	...
	case 'maps':
        ...
        case 'broad':
            ...
            for (let ids of chatz) {
                ...
                if (!cvk.isReadOnly) await kill.sendText(ids, `[Transmissão do dono da Kaiser bot]\n\n${msg}`)
            }
